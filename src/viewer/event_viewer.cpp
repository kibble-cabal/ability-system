...
void EventViewer::add_header(
    RenderContainer *container, Ref<AbilityEvent> event
) const {
    Ref<Ability> ability = event->get_ability();
    String header = ability.is_valid()
                        ? "Event: " + String(ability->get_identifier())
                        : "Event: <null>";
    container->add_label(
        header,
        ability.is_valid() ? ability->get_ui_color() : Color(0, 0, 0, 0)
    );
}

void EventViewer::add_effects(
    RenderContainer *container, Ref<AbilityEvent> event
) const {
    ...
    for (int i = 0; i < effects.size(); i++) {
        ...
        // If the instance does not exist, it's already been finished and
        // removed.
        Status status = instance.is_valid() ? instance->get_last_status()
                                            : Status::FINISHED;
        String text
            = fmt("{0}({1})", effect->get_class(), effect->get_ui_name());
        // For looped effects, add a counter.
        if (instance.is_valid() && instance->is_loop_effect())
            text += stringify(
                " (loop ",
                (int)(instance->get("elapsed_loops")) + 1,
                ")"
            );
        switch (status) {
            case Status::RUNNING:
            ...
            case Status::FINISHED:
                container->add_text_strike_label(
                    text,
                    Color(effect->get_ui_color(), 0.5)
                );
                break;
            default:
                container->add_text_label(
                    text,
                    Color(effect->get_ui_color(), 0.5)
                );
        }
        ...
        // Add "→" or "&" icon
        if (i < (effects.size() - 1)) {
            String icon = ability->get_effect_mode() == EffectMode::PARALLEL
                              ? " & "
                              : String::utf8(" → ");
            container->add_text_label(
                icon,
                Color(ability->get_ui_color(), 0.5)
            );
        }
    }
}
...
void EventViewer::_draw() {
    ...
    for_each(ability_system->get_events(), [&](Ref<AbilityEvent> event) {
        ...
        if (event->get_ability().is_valid())
            container.draw_outline(
                Color(event->get_ability()->get_ui_color(), 0.5)
            );
...
Vector2 EventViewer::_get_minimum_size() const {
    ...
    for_each(get_ability_system()->get_events(), [&](Ref<AbilityEvent> event) {
        RenderContainer container
            = make_container_const(total_rect.size.height);
        add_header(&container, event);
        container.new_line();
        add_effects(&container, event);
