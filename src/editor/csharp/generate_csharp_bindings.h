...
class GenerateCSharpBindingsPlugin {
    ...
    struct DefaultData {
        ...
        DefaultData(
            const ArgumentData &arg, bool need_remap, const String &_arg_str, bool nullable = false
        )
            : name(arg.name),
        ...
        bool is_equal_data(const DefaultData &other) const {
            return type_name == other.type_name && arg_string == other.arg_string
                   && is_need_remap == other.is_need_remap && is_nullable == other.is_nullable;
        }
    };
...
private:
    ...
    StringName get_native_base_class(const StringName &cls) const {
        ...
        ignored_classes.append("AbilitySystemViewerBase");
        while (true) {
            auto base = ClassDB::get_parent_class(current);
            if (!ignored_classes.has(base)) return base;
            else current = base;
        }
        return current;
    }
    ...
    std::vector<DefaultData> arguments_parse_values(
    ...
    String arguments_string_decl(
        const TypedArray<Dictionary> &args,
        bool with_defaults,
        std::vector<DefaultData> def_args_data = {}
    );
    String arguments_string_call(
