...
AttributeInspectorEditorRow::AttributeInspectorEditorRow() {
    ...
    name_label->set_vertical_alignment(
    ...
    range_label->set_vertical_alignment(
        VerticalAlignment::VERTICAL_ALIGNMENT_CENTER
    );
...
void AttributeInspectorEditorRow::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_on_spin_box_changed", "value"),
        &AttributeInspectorEditorRow::_on_spin_box_changed
    );
    ClassDB::bind_method(
        D_METHOD("_on_remove_pressed"),
        &AttributeInspectorEditorRow::_on_remove_pressed
    );
}
...
void AttributeInspectorEditorRow::_ready() {
    remove_button->connect(
        "pressed",
        callable_mp(this, &AttributeInspectorEditorRow::_on_remove_pressed)
    );
    spin_box->connect(
        "value_changed",
        callable_mp(this, &AttributeInspectorEditorRow::_on_spin_box_changed)
    );
...
void AttributeInspectorEditorRow::set_attribute(Ref<Attribute> attribute) {
    ...
    range_label->set_text(
        String("({0} - {1}) ").format(variant_array(min_value, max_value))
    );
    range_label->add_theme_color_override(
        "font_color",
        Color(contrast_color, 0.5)
    );
    range_label->set_anchors_and_offsets_preset(PRESET_RIGHT_WIDE);
...
void AttributeInspectorEditorRow::_on_remove_pressed() {
    if (attribute.is_valid() && on_remove != nullptr) on_remove(attribute);
}