...
void AbilityEvent::_bind_methods() {
    ...
    /* Bind signals */
    ADD_SIGNAL(
        MethodInfo(as_signal::Started, OBJECT_PROP_INFO(AbilitySystem, owner))
    );
    ADD_SIGNAL(
        MethodInfo(as_signal::Finished, OBJECT_PROP_INFO(AbilitySystem, owner))
    );
    ADD_SIGNAL(MethodInfo(
        as_signal::EffectStarted,
        OBJECT_PROP_INFO(AbilitySystem, owner),
        OBJECT_PROP_INFO(Effect, effect_instance)
    ));
    ADD_SIGNAL(MethodInfo(
        as_signal::EffectFinished,
        OBJECT_PROP_INFO(AbilitySystem, owner),
        OBJECT_PROP_INFO(Effect, effect_instance)
    ));
}

void AbilityEvent::start_effect(
    AbilitySystem *owner, Ref<Effect> effect_instance
) {
...
void AbilityEvent::finish_effect(
    AbilitySystem *owner, Ref<Effect> effect_instance
) {
...
void AbilityEvent::start(AbilitySystem *owner) {
    ...
    for_each_i(
        this->ability->get_effects(),
        [this, owner](Ref<Effect> effect, int i) {
...
Status AbilityEvent::tick(AbilitySystem *owner, float delta) {
    ...
    if (effect_instances.size() > 0
        && ((Ref<Effect>)effect_instances[0])->is_loop_effect()) {
...
Ref<Effect> AbilityEvent::get_effect_instance(Ref<Effect> effect, int index)
    const {
